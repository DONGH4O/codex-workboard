...
恢复演练应使用第二个隔离目录启动目录包，再核对任务数量、泳道、分类、执行记录、审计动作和附件。实际启动及物理界面检查属于单独授权的人工验收。
